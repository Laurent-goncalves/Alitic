...
class FoodLayout : ConstraintLayout {
    ...
    private fun removeFood(view: View, onFoodToDeleteListener: OnFoodToDeleteListener) {
        onFoodToDeleteListener.onFoodToDelete(view.findViewById<TextView>(R.id.food_name).text.toString())
    }

    fun addFood(food: Food, onFoodToDeleteListener: OnFoodToDeleteListener): FoodViewId? {

        val view = getFoodView(food, onFoodToDeleteListener)
        if (view != null) {
            this.addView(view,0)
    ...
    private fun getFoodView(food:Food, onFoodToDeleteListener: OnFoodToDeleteListener):View?{
        ...
        if (inflater != null){
            ...
            view.setOnClickListener { removeFood(view, onFoodToDeleteListener) }
        ...
        while(list.isNotEmpty()){
            ...
            if(list.isNotEmpty()){
                for(i in 0 until list.size){
                    if(sumChar + list[i].nameFood.length + 5 <= 15){
